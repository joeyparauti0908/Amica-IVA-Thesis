# Experiment Version IVA Joey Parauti
...
strategies_df = pd.read_csv('promoting_strategies.csv', encoding='utf-8')

# Function to randomly select a strategy category and then a strategy from that category
def get_random_strategy():

    # Choose one of the three main strategy categories at random
    selected_category = random.choice(strategies_df['Category'].unique().tolist())

    # Filter the DataFrame to only include strategies from the selected category
    category_subset = strategies_df[strategies_df['Category'] == selected_category]

    # Randomly select one strategy (row) from the filtered category
    selected_row = category_subset.sample(n=1).iloc[0]

    # Extract the strategy description (For the Mistral Prompt)
    strategy_description = selected_row['Description']
...
# Ask Mistral to suggest an activity based on promotion strategy
def mistral_activity_prompt(role, query_client, conversation_history, language, activity, strategy_description):
    prompt = (
        f"Use the following suggestion strategy when formulating your message: {strategy_description}."
        f"Propose the following activity: {activity} based on the suggestion strategy. Make sure it stay in a suggestive way."
    ...
    generated_answer = query_mistral(prompt, query_client, role)
    conversation_history.append({"activity": activity, "strategy_description": strategy_description, "answer": generated_answer})
    return generated_answer
...
def dialogue_chitchat(text_strings, display_strings, language, gender):
    ...
    while state != "end":
        ...
        if state == "start":
            ...
            strategy_description = get_random_strategy()
            # Log the strategy description
            with open("strategy_log.txt", "a", encoding="utf-8") as log_file:
                log_file.write(strategy_description + "\n")
            conversation_history.append({"role": "user", "content": activity})

            try:
                response = mistral_activity_prompt(role, client, conversation_history, language_adjective, activity, strategy_description)
            except:
